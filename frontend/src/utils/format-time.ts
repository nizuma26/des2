...
//output
export const fYear = (birthDate: DatePickerFormat | string): [number, string] | string => {
  try {

    const today = dayjs(); // Obtener fecha actual
    const birth = dayjs(birthDate); // Convertir la fecha que se recibe a un objeto dayjs
    const age = today.diff(birth, 'year'); // Calcular la diferencia en años
    return [age, 'años'];
