...
import { deleteItem } from '../../../../api/inventory/item';
...
export default function PopupOptions({
  ...
  const handleDelete = () => {
    showAlert({
      content: `¿Esta seguro de eliminar el artículo "${item}" ?`,
      icon: 'solar:trash-bin-minimalistic-bold',
      fn: () =>
        mutate.submit({
          promise: deleteItem(id),
          onSuccess: () => {
  ...
  return (
    <>
      ...
      >        
        <MenuItem onClick={() => router.replace(`/item/edit/${id}`)} sx={{ px: '9px' }}>
          <Iconify
